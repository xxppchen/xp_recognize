...
# 原始数据读取，后续新增标签数据，
feature_data = pd.read_csv('D:/Desktop/项目/负荷识别部/dataset/公司数据/Feature_Data_0719(-10).csv')
feature_data.sort_values("file_number", inplace=True)
...
# 聚类总标签
feature_data['cluster1-4'] = feature_data.apply(
    lambda row: str(my_point2(row['cluster1'], row['cluster2'], row['cluster3'], row['cluster4'])), axis=1)
...
def get_P_bins(P_list, P_bins):
    P_bins_list = []
    for p in P_list:
        for i in range(len(P_bins)):
            if p <= P_bins[i]:
                P_bins_list.append(i)
                break
            if p > P_bins[len(P_bins) - 1]:
                P_bins_list.append(len(P_bins))
                break
    return P_bins_list


P_list = list(feature_data["P"])
P_bin_list = get_P_bins(P_list, [18.951, 41.474, 130.388, 572.396])
...
for brand in unique_brand:
    if len(unique_brand[brand]) > 1:
        ...
        feature_data.loc[feature_data["brand"] == unique_brand[brand][random.randrange(1, a)], "Is_test"] = 1
...
input_data = pd.DataFrame({
    "file_name": feature_data["file_number"],
    "P_bins": P_bin_list,
    "P": feature_data["P"],
...
input_data.to_csv("../Hioki/Input_Data0720.csv", index=False, sep=',')
print("XXX")
